...
fitsfiles={'filename':'chanc18o_forks.pdf',
...
for startchan in range(firstchannelstart,lastchannel,ypanels*xpanels):
    ...
    for j in range(0,ypanels): # this order first follows row
        for i in range(0,xpanels):
            ...
            if j == 1: 
                if i == 0: 
                    templatehdulist.writeto('channel_8p18.fits',output_verify='exception',clobber=True,checksum=False) # use this as template to output every single channel
                if i == 1: 
                    templatehdulist.writeto('channel_8p40.fits',output_verify='exception',clobber=True,checksum=False) # use this as template to output every single channel

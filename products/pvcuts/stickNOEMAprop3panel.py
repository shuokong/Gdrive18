...
for j in range(0,ypanels):
    for i in range(0,xpanels):
        ...
        if panelnum == xpanels*ypanels:
             ff.show_regions('stick6LanecoresElli.reg')
...
#            cb1.ax.set_yticklabels(['{:<.1f}'.format(colorticks[ii]/1.e23) for ii in range(len(colorticks))]) # 
        ...
        ff.add_label(xcenter - 0.9 * wid / 2.,ycenter + 0.9 * hei / 2.,'('+lletter[panelnum-1+3]+')',color='white',horizontalalignment='center')
